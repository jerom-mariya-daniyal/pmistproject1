...
//     setLoading(true);
//     try {
//       const response = await api.post('/tasks', form, token);
      
//       if (response.emailSent) {
//         alert('✅ Task assigned successfully!\n📧 Email notification sent to ' + form.assignedTo);
//       } else {
//         alert('✅ Task assigned successfully!\n⚠️ Email notification failed to send (task still created)');
//       }
...
//                     <span className="text-sm font-bold text-blue-600 bg-blue-100 px-3 py-1 rounded-xl shadow-lg">{t.naacId}</span>
//                     {t.emailSent && (
//                       <span className="text-xs font-bold text-green-600 bg-green-100 px-2 py-1 rounded-lg shadow-lg flex items-center space-x-1" title="Email notification sent">
...
const api = {
  post: async (url, data, token) => {
    const h = { 'Content-Type': 'application/json' };
    if (token) h.Authorization = 'Bearer ' + token;
    const r = await fetch(API_URL + url, { method: 'POST', headers: h, body: JSON.stringify(data) });
    return r.json();
  },
  get: async (url, token) => {
    const h = {};
  ...
  put: async (url, data, token) => {
    const h = { 'Content-Type': 'application/json' };
    if (token) h.Authorization = 'Bearer ' + token;
    const r = await fetch(API_URL + url, { method: 'PUT', headers: h, body: JSON.stringify(data) });
    return r.json();
  },
  delete: async (url, token) => {
    const h = {};
    if (token) h.Authorization = 'Bearer ' + token;
    const r = await fetch(API_URL + url, { method: 'DELETE', headers: h });
    return r.json();
  },
  upload: async (url, formData, token) => {
    const h = {};
    if (token) h.Authorization = 'Bearer ' + token;
    const r = await fetch(API_URL + url, { method: 'POST', headers: h, body: formData });
    return r.json();
  }
};
